...
import org.bson.util.BytesBSON;
...
class JSONParser {
    ...
    public int getSubStringNum(String s, String sub) {
    	int i = s.length() - s.replace(sub, "").length();
    	return (i / sub.length());
    }
    /**
     * Parses an object for the form <i>{}</i> and <i>{ members }</i>.
    ...
    protected Object parseObject(final String name) {
        ...
        while (get() != '}') {
            ...
            int regexNum = getSubStringNum(value.toString(), "/");
            if (key.equals("$regex") && value!=null) {
            	doRegexOptions(value.toString());
            }else if(regexNum==2) {
            	value=doLikeOptions(value.toString());
            	_callback.gotRegex(key, (Pattern)value);
            }else {
    ...
    private void doRegexOptions(String value) {
    	String ret = value;
    	int regexNum = getSubStringNum(value.toString(), "/");
    	if (regexNum==2) {
    		ret = parseRegexVal(value).toString();  
    		doCallback("$regex", ret);
        	int end2=value.lastIndexOf("/");
        	//$options
        	String opt = value.substring(end2+1);
        	if(opt!=null && !opt.isEmpty()) {
	    		doCallback("$options", opt);
	    	}
    	}else {
    		doCallback("$regex", ret);
    	}
    ...
    private Object parseRegexVal( String value) {
    	int start = 0, end = 0;
    	Object ret = value;
    	start=value.indexOf("/^");
    	int end2=value.lastIndexOf("/");
    	...
    	if (start>-1 && end>-1) {
    		ret = value.substring(1, end+1);
    	}else {
    		if(start>-1) {
    			ret = value.substring(1, end2+1).replace("/", ".*");
	    	}else if(end>-1) {
		    	ret = value.substring(0, end+1).replace("/", ".*");
	    	}else {
	    		ret = value.substring(1, end2+1).replaceAll("/", ".*");
	    	}
    ...
    private Object doLikeOptions( String value) {
    	Object ret = parseRegexVal(value);
    	
    	int end2=value.lastIndexOf("/");
    	//$options
    	String opt = value.substring(end2+1);
    	if(opt!=null && !opt.isEmpty()) {
    		ret = Pattern.compile(ret.toString(), BytesBSON.regexFlags(opt));
    	}else {
    		ret = Pattern.compile(ret.toString());
    	}
    	return ret;
    }
