...
abstract class XmlaOlap4jStatement implements OlapStatement {
    ...
    public CellSet executeOlapQuery(String mdx) throws OlapException {
        ...
        if ("SAP_BW".endsWith(dataSourceInfo) )
        {
        	try {
	        	byte[] bt = openCellSet.populateSap();
	        	if (1==2){ //pamire for save xml
		        ...
		        localSAXParser.parse(bais, localXMLASaxParser);
...
//		        List<String> axis1 = localXMLASaxParser.getAxis1();
		        ...
		        mappedByteBufferTable.setColumn(cols);

			    openCellSet.setDataBuf(mappedByteBufferTable);
        	} catch (Exception e) {
