...
public abstract class XmlaOlap4jConnection implements OlapConnection {
    ...
    public static enum BackendFlavor {
        ...
        public static BackendFlavor getFlavor(XmlaOlap4jConnection conn)
            throws OlapException
        {
            //final String dataSourceInfo = conn.getOlapDatabase().getDataSourceInfo();
        	final String dbName = conn.getDatabase();
            for (BackendFlavor flavor : BackendFlavor.values()) {
                if (dbName.contains(flavor.token)) {
                    return flavor;
                }
            }
